...
class Eeprom24C 
{
    public:
        ...
        void write_string(unsigned int address, const char* str) {
            int length = strlen(str);
            for (int i = 0; i < length; i++) {
                write_1_byte(address + i, str[i]);
            }
            // Ghi ký tự null (0) cuối cùng để đánh dấu kết thúc chuỗi
            write_1_byte(address + length, 0);
        }

        String read_string(unsigned int address) {
            String result = "";
            char ch;
            int i = 0;
            do {
                ch = read_1_byte(address + i);
                if (ch != 0) {
                    result += ch;
                }
                i++;
